...
instruction_t* decode_instructions(unsigned int* bytes, unsigned int
num_instructions);
unsigned int execute_instruction(unsigned int program_counter, instruction_t*
instructions,
int* registers, unsigned char* memory);
void print_instructions(instruction_t* instructions, unsigned int
num_instructions);
...
instruction_t* instructions = decode_instructions(instruction_bytes,
num_instructions);
...
unsigned char* memory = malloc(sizeof(unsigned char) * STACK_SIZE);
// Run the simulation
...
{
program_counter = execute_instruction(program_counter, instructions, registers,
memory);
}
return 0;
}
...
retval[i].opcode = (bits & 0xF8000000) >> 27;
retval[i].first_register= (bits & 0x07C00000) >> 22;
retval[i].second_register = (bits & 0x003E0000) >> 17;
retval[i].immediate = (int16_t)(bits & 0x0000FFFF);
}
return retval;
}
/*
...
unsigned int execute_instruction(unsigned int program_counter, instruction_t*
instructions, int* registers, unsigned char* memory)
...
// Flags are in register 16
unsigned CF = registers[16] & 1;
unsigned ZF = (registers[16] >> 6) & 1;
unsigned SF = (registers[16] >> 7) & 1;
unsigned OF = (registers[16] >> 11) & 1;
switch(instr.opcode)
{
...
break;
case shrl:
registers[instr.first_register] = (int)
((uint32_t)registers[instr.first_register] >> 1);
break;
case movl_reg_reg:
registers[instr.second_register] = registers[instr.first_register];
break;
case movl_deref_reg:
registers[instr.second_register] = *(int*)
...
break;
case movl_imm_reg:
registers[instr.first_register] = (int32_t)(int16_t)instr.immediate;
break;
case cmpl:
if ((unsigned int)registers[instr.second_register] < (unsigned
int)registers[instr.first_register])
{
CF = 1;
}
else
...
if ((unsigned int)registers[instr.second_register] - (unsigned
int)registers[instr.first_register] == 0)
{
ZF = 1;
}
else
...
if ((((unsigned int)registers[instr.second_register] - (unsigned
int)registers[instr.first_register]) & 0x80000000) != 0)
{
SF = 1;
}
else
...
(registers[instr.second_register] ^ (registers[instr.second_register] -
registers[instr.first_register]))) & 0x80000000) != 0)
{
OF = 1;
}
else
...
if(ZF == 1)
{
registers[16] = (OF * 2048) + (SF * 128) + (ZF * 64) + (CF);
return program_counter + 4 + (int32_t)instr.immediate ;
}
break;
case jl:
if(SF ^ OF)
...
error_exit("unable to allocate memory for instruction bytes (something went
really wrong)");
